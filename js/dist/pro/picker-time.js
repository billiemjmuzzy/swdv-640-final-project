jQuery(function ($) {
  var $win = $(window);
  var $doc = $(document); // Can I use inline svg ?
  ...
  var svgSupported = 'SVGAngle' in window && function () {
    ...
    el.innerHTML = '<svg/>';
    var supported = (el.firstChild && el.firstChild.namespaceURI) == svgNS;
    el.innerHTML = '';
    return supported;
  ...
  var mouseupEvent = "mouseup.clockpicker " + (touchSupported ? ' touchend.clockpicker' : ''); // Vibrate the device if supported
  ...
  function ClockPicker(element) {
    ...
    this.minutesBeforeChange = null; // round minutestep to multiples of five

    if (options.minutestep > 20) {
      options.minutestep = 20;
    } else if (options.minutestep != 1 && options.minutestep % 5) {
      var rest = options.minutestep % 5;
    ...
    this.input.on('focus', function () {
      return _this.icon.addClass('active');
    });
    this.input.on('blur', function () {
      return _this.icon.removeClass('active');
    });

    var parseTimeOptions = function parseTimeOptions(scope) {
      var time = scope + 'Time';
      _this[time] = options[scope].split(':'); // user can set using am/pm. Convert 12h clock settings to 24h clock settings 

      if (_this[time][1].length === 4) {
        var period = _this[time][1].replace(/\d+/g, '').toUpperCase();

        _this[time][1] = _this[time][1].replace(/\D+/g, '');
        if (period === 'PM') _this[time][0] = parseInt(_this[time][0]) + 12;
      }

      if (_this[time].length !== 2) _this[time] = null;else {
        for (var _i = 0; _i < _this[time].length; _i++) {
          _this[time][_i] = +_this[time][_i];
        }
      }
    ...
    if (options.max) {
      parseTimeOptions('max'); // Ensure that max is after min, if not remove the max it's invalid
    ...
    if (options.darktheme) popover.addClass('darktheme'); // If autoclose is not setted, append a button
    ...
    this.spanMinutes.click($.proxy(this.toggleView, this, 'minutes')); // Build ticks
    ...
    function mousedown(e, space) {
      ...
      if (svgSupported) plate.append(self.canvas); // Clock
      ...
      $doc.off(mouseupEvent).on(mouseupEvent, function (e) {
        ...
        if (self.isInvalidTimeScope) {
          ...
          e.stopPropagation();
        } else if (self.currentView === 'hours') self.toggleView('minutes', duration / 2);else if (self.currentView != 'hours' && options.autoclose) {
    ...
    var setTabTrap = function setTabTrap() {
      popover.find('.clockpicker-span-hours').on('keydown', function (e) {
        if (e.keyCode === tab && e.shiftKey) {
          e.preventDefault();
          popover.find('.done-button').focus();
        }
      });
      popover.find('.done-button').on('keydown', function (e) {
        if (e.keyCode === tab && !e.shiftKey) {
          e.preventDefault();
    ...
    var setIconTabindex = function setIconTabindex() {
      icon.attr('tabindex', '0');
    };

    var setAriaInputAndIcon = function setAriaInputAndIcon() {
      icon.attr('aria-haspopup', 'true');
    };

    var isNewValueOutOfRange = function isNewValueOutOfRange(val) {
      var hours = self.hours,
          isTwelvehour = self.isTwelvehour,
          amOrPm = self.amOrPm,
    ...
    var bindIconKeydown = function bindIconKeydown() {
      icon.on('keydown', function (e) {
        if (e.keyCode === space || e.keyCode === enter) {
          debounce(_this.show(), 100);
        }
      });
    };

    var bindHoursSpanKeydown = function bindHoursSpanKeydown() {
      _this.spanHours.on('keydown', function (e) {
        var newValue;

        if (e.keyCode === arrowUp) {
          if (_this.currentView !== 'hours') {
            _this.toggleView('hours');
          }

          if (_this.isTwelvehour) {
            if (_this.hours === 11) {
              newValue = _this.hours + 1;
              var period = _this.amOrPm === 'AM' ? 'PM' : 'AM';
              self.togglePeriod(period);
            } else if (_this.hours === 12) {
              newValue = 1;
            } else {
              newValue = _this.hours + 1;
            }
          ...
          if (isNewValueOutOfRange(newValue)) {
            if (_this.isTwelvehour && _this.minHours <= 12) {
              self.togglePeriod('AM');
            }

            newValue = _this.minHours;
          }

          _this.hours = newValue;

          _this.spanHours.html(leadingZero(_this.hours));
        ...
        if (e.keyCode === arrowDown) {
          ...
          if (_this.isTwelvehour) {
            if (_this.hours === 1) {
              ...
              var _period = _this.amOrPm === 'AM' ? 'PM' : 'AM';

              self.togglePeriod(_period);
            } else {
              newValue = _this.hours - 1;
            }
          } else {
          ...
          if (isNewValueOutOfRange(newValue)) {
            if (_this.isTwelvehour && _this.maxHours >= 12) {
              self.togglePeriod('PM');
            }

            newValue = _this.maxHours;
          }
    ...
    var bindMinutesSpanKeydown = function bindMinutesSpanKeydown() {
      _this.spanMinutes.on('keydown', function (e) {
        var newValue;

        if (e.keyCode === arrowUp) {
          if (_this.currentView !== 'minutes') {
            _this.toggleView('minutes');
          }

          if (_this.minutes === 59) {
            newValue = 0;
          } else {
            newValue = _this.minutes + 1;
          }

          if (isNewValueOutOfRange(newValue)) {
            var hours = _this.hours,
                amOrPm = _this.amOrPm,
                minHours = _this.minHours,
                maxHours = _this.maxHours,
                minMinutes = _this.minMinutes;
        ...
        if (e.keyCode === arrowDown) {
          ...
          if (isNewValueOutOfRange(newValue)) {
            var _hours = _this.hours,
                _amOrPm = _this.amOrPm,
                _minHours = _this.minHours,
                _maxHours = _this.maxHours,
                maxMinutes = _this.maxMinutes;
    ...
    var bindAmPmBlockKeydown = function bindAmPmBlockKeydown() {
      if (_this.isTwelvehour) {
        popover.find('.am').on('keydown', function (e) {
          if (e.keyCode === enter) {
            e.preventDefault();
            self.togglePeriod('AM');
          }
        });
        popover.find('.pm').on('keydown', function (e) {
          if (e.keyCode === enter) {
            e.preventDefault();
            self.togglePeriod('PM');
          }
    ...
    var bindCloseButtonClick = function bindCloseButtonClick() {
      popover.find('.close-button').on('click', function () {
        _this.close();
      });
    };

    var bindShowOnClick = function bindShowOnClick() {
      var el;
      ...
      el.on('click.clockpicker', debounce($.proxy(_this.show, _this), 100));
    };
  ...
  ClockPicker.prototype.parseInputValue = function () {
    ...
    value = value.split(':'); // Minutes can have AM/PM that needs to be removed
  ...
  ClockPicker.prototype.show = function (e) {
    ...
    this.disableOutOfRangeElements(); // Toggle to hours view

    this.toggleView('hours'); // Set position

    this.locate();
    this.isShown = true;
    this.spanHours.focus(); // Hide when clicking or tabbing on any element except the clock and input
  ...
  ClockPicker.prototype.disableOutOfRangeElements = function () {
    ...
    var $hoursTick = $('.clockpicker-hours').children(),
        $minutesTick = $('.clockpicker-minutes').children(),
        $amBtn = $('.am'),
        $pmBtn = $('.pm'),
        $doneBtn = $('.done-button'); // disable am/pm switch btn when time is out of allow range for am/pm period

    if (isTwelvehour && currentView === 'minutes') {
      $doneBtn.removeClass('grey-text disabled');
      if (amOrPm === 'AM' && !(hours + 12 <= maxHours) && options.max) $pmBtn.addClass('disabled');else if (amOrPm === 'PM' && !(hours >= minHours) && options.min) $amBtn.addClass('disabled');
    ...
    if (currentView === 'minutes') {
      if (hours === minHours && minutes < minMinutes || hours === maxHours && minutes > maxMinutes) $doneBtn.addClass('grey-text disabled');else $doneBtn.removeClass('grey-text disabled');
    } // set invalid if hours is out of allow range


    if (currentView === 'hours') {
      $hoursTick.each(function (index, el) {
        var hours = el.innerHTML;
        if (_this2.isTwelvehour && hours == 12) hours = 0;
        if (hours > maxHours || hours < minHours) $(el).addClass('grey-text disabled');else $(el).removeClass('grey-text disabled');
      });
    } // set invalid if minutes is out of allow range


    if (currentView === 'minutes') {
      $minutesTick.each(function (index, el) {
        if (minHours == hours && el.innerHTML < minMinutes) $(el).addClass('grey-text disabled');else if (maxHours == hours && el.innerHTML > maxMinutes) $(el).addClass('grey-text disabled');else if (el.innerHTML % _this2.options.minutestep !== 0) $(el).addClass('grey-text disabled');else $(el).removeClass('grey-text disabled');
      });
    }
  ...
  ClockPicker.prototype.setHand = function (x, y, roundBy5, dragging) {
    ...
    if (options.twelvehour) radius = outerRadius; // Radian should in range [0, 2PI]
    ...
    if (isHours) {
      ...
      var _hours2 = this.hours;
      if (amOrPm === 'PM') _hours2 += 12;

      if (_hours2 == minHours && value < minMinutes || _hours2 == maxHours && value > maxMinutes) {
        this.isInvalidTimeScope = true;
        return;
      }
    }
    ...
    if (this[this.currentView] !== value) if (vibrate && this.options.vibrate) // Do not vibrate too frequently
  ...
  ClockPicker.prototype.clearInput = function () {
    this.input.val('');
    this.hide();
    if (this.options.afterDone && typeof this.options.afterDone === 'function') this.options.afterDone(this.input, null);
  ...
  $.fn.timepicker = function (option) {
    ...
    if (this.length == 1) {
      var returnValue = handleClockPickerRequest.apply(this[0]); // If we do not have any return value then return the object itself so you can chain
  ...
  $('#time-picker-opener').on('click', function (e) {
    e.stopPropagation();
    e.preventDefault();
    var openElementData = e.target.dataset.open;
    var $picker = $("#" + openElementData).timepicker('picker');
